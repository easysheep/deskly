...
const ProjectsList: React.FC<ProjectsListProps> = ({ org_id }) => {
  ...
  useEffect(() => {
    const fetchProjectsAndTasks = async () => {
      try {
        const projectsResponse = await fetch(`/api/projectlist?org_id=${org_id}`);
          ...
          {projects.map((project) => (
            ...
            >
              ...
              {deleteBoxVisible === project.project_id && (
                <div className="absolute top-8 right-2 bg-white shadow-lg p-2 rounded-lg z-10" onClick={(e) => e.stopPropagation()}>
                  <button
                      ...
                      if (role !== "admin") {
                        toast.error(
                          <div className="text-red-500 font-bold text-center">
                            Access Denied! <br /> You do not have admin privileges.
                          </div>,
                          { duration: 3000 }
                        );
                        return;
              ...
              <div className="w-full bg-white bg-opacity-30 rounded-full h-2 mt-4">
                <div
                  ...
                  style={{ width: `${calculateCompletion(project.project_id)}%` }}
                ></div>
              </div>
